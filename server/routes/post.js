...
async function appendExtra(posts, options = {}) {
  ...
  return posts.map(post => {
    return {
      ...post,
      extra: {
        liked: options.viewer ? likes.find(like => like.to === post.trxId)?.value > 0 : false,
        profile: profiles.find(profile => profile.userAddress === post.userAddress) || { name: post.userAddress },
        likeCount: _.sumBy(likes.filter(like => like.to === post.trxId), like => like.value),
        commentCount: comments.filter(comment => comment.to === post.trxId).length
      }
    }
  });
}
