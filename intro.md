...
- 我运行一个 Rum 节点 B
- 我们两个节点互相连接、互相同步数据
- 那么数据就是我们自己掌握的，而且你我都存储了一份

这是 Rum 能提供的基础功能。

听起来有点像是分布式数据库，对吧。当你自己这份数据丢了，可以从别人那里拿回来。所以，你确实可以这么理解它。

作为开发者，我们可以基于这个做什么呢？举例来说，假如你要做一个产品，那么你可以自己跑一个 Rum 节点，然后你把这个产品的所有数据都存储在 Rum 节点里面。

假如你这个 Rum 节点没有连接其他人，那么就是单机版，也就是说，数据存储在你个人电脑上，弄没了就是没了。这并不是去中心化的，和传统的数据库没有什么区别。

所有只有当多个 Rum 节点互相同步，才是真正的去中心化。也就是变成了分布式的数据库，对吧。

从产品开发的角度，把 Rum 节点类比成分布式数据库，是合适的。那么接下来我们就可以了解一下，如何操作这个“数据库”，也就是说，如何写数据和读数据。

这里我们还需要了解另外一个东西 -- Group。

假如你跑了一个 Rum 节点，你就可以根据需要，在节点上你就可以建立 group。简单解释一下就是：

1. 节点和 groups 是一对多的关系
2. 一个节点上面可以建立无数个 groups
3. 每个 group 都有一个 seed（种子）
4. 我们通过 seed 来使用 group、同步 group、加入 group
5. 一般我们开发一个 app，都是建一个 group，然后把数据都存储在里面，就足够了
6. 你可以把 group 类比我们平时使用的云服务的对象存储，你建立一个 bucket，然后把数据都存储在里面
...
这就是写和读的方法，具体的使用说明可以看 [Rum JS SDK](https://github.com/okdaodine/quorum-light-node-sdk)

那么现在的问题就是如何通过写不同的数据，以实现你想要的功能，对吧？

这个问题我觉得最好使用例子来说明，[rum-demo](https://github.com/okdaodine/rum-demo) 就是这样一个真实的例子，你通过使用它以及阅读它的代码实现，应该就可以弄明白整个来龙去脉。
